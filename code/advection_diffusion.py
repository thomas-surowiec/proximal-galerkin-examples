...
def exact_solution(x, epsilon):
    ...
    u_exact = (exp(r2 * (x[0] - 1.0)) - exp(r1 * (x[0] - 1.0))) * cos(
        pi * x[1]
    ) / denom + 0.5
    return u_exact
...
def BG_solve(
    ...
    if Version(dolfinx_version) >= Version("0.10.0"):
        int_points = V.element.interpolation_points
    else:
        int_points = V.element.interpolation_points()

    u_bc = fem.Function(V)
    u_bc.interpolate(fem.Expression(u_exact, int_points))
    ...
    H1_error_form = fem.form(
        epsilon * inner(grad(uh - u_exact), grad(uh - u_exact)) * dx
        + (uh - u_exact) ** 2 * dx
    )
...
def PG_solve(
    ...
    if Version(dolfinx_version) >= Version("0.8.0"):
        ...
        Q = basix.ufl.element("Lagrange", msh.basix_cell(), polynomial_order)
        mixed_element = basix.ufl.mixed_element([Q, Q])
    ...
    if Version(dolfinx_version) >= Version("0.10.0"):
        int_points = V.sub(0).element.interpolation_points
    else:
        int_points = V.sub(0).element.interpolation_points()
    u_bc.interpolate(fem.Expression(u_exact, int_points))
    ...
    if Version(dolfinx_version) >= Version("0.10.0"):
        int_points1 = V.sub(0).element.interpolation_points
    else:
        int_points1 = V.sub(0).element.interpolation_points()
    psi_bc.interpolate(fem.Expression(lnit_u_exact, int_points1))
    ...
    L2latent_increment_form = fem.form(
        ((tanh(psi / 2) + 1) / 2 - (tanh(psi_k / 2) + 1) / 2) ** 2 * dx
    )
    H1primal_error_form = fem.form(
        epsilon * inner(grad(u - u_exact), grad(u - u_exact)) * dx
        + (u - u_exact) ** 2 * dx
    )
    ...
    sol.sub(0).interpolate(uh)
    sol.sub(1).interpolate(fem.Expression(lnit_uh, int_points1))
    ...
    for k in range(maximum_number_of_outer_loop_iterations):
        ...
        if increment_k > 0.0:
            rank_print(
                f"Increment size: {primal_increment}"
                + f"   Ratio: {primal_increment / increment_k}",
                msh.comm,
            )
        else:
    ...
    with io.VTXWriter(msh.comm, output_dir / "u.bp", [sol.sub(0).collapse()]) as vtx:
        vtx.write(0.0)

    # Export interpolant of exact solution u
    expr = fem.Expression(u_exact, int_points)
    V_primal, _ = V.sub(0).collapse()
    q = fem.Function(V_primal)
    q.interpolate(expr)
    with io.VTXWriter(msh.comm, output_dir / "u_exact.bp", [q]) as vtx:
        vtx.write(0.0)

    # Export latent solution variable
    with io.VTXWriter(msh.comm, output_dir / "psi.bp", [sol.sub(1).collapse()]) as vtx:
        vtx.write(0.0)

    # Export feasible discrete solution
    exp_psi = (tanh(sol.sub(1) / 2) + 1) / 2
    expr = fem.Expression(exp_psi, int_points)
    q.interpolate(expr)
