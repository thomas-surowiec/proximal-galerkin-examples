...
//
//
// Add file to mfem/examples and compile with: make discontinuous_obstacle
//
// Sample runs: mpirun -np 4 discontinuous_obstacle
//
//
...
real_t discontinuous_obstacle(const Vector &pt);
...
int main(int argc, char *argv[])
{
   ...
   *nodes /= scale;
   ...
   FunctionCoefficient obstacle(discontinuous_obstacle);
   CoefficientRefiner  coeffrefiner(obstacle, order);
   coeffrefiner.SetThreshold(refiner_threshold);
   int out = coeffrefiner.PreprocessMesh(pmesh);
